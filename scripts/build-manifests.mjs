...
(function build(){
  ...
  // New Features
  for (const l of LOCALES) {
    const dir = path.join(CONTENT, 'new-features')
    const json = path.join(dir, `${l}.json`)
    const md = path.join(dir, `new-features.${l}.md`)
    const txt = path.join(dir, `new-features.${l}.txt`)

    if (fs.existsSync(json)) {
      const parsed = safeParse(rd(json), [])
      const normalized = Array.isArray(parsed) ? parsed : []
      wj(`new-features.${l}.json`, normalized)
    } else {
      const body = rd(fs.existsSync(md) ? md : txt)
      wj(`new-features.${l}.json`, { body })
    }
  }
